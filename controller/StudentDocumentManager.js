...
const uploaddoc = async (req, res, next) => {
    try {
        ...
        const uploadPromises = req.files.map(async (file) => {
            await uploadFile(authClient, file.buffer, `${req.query.student}-${file.originalname}`, req.user.InstutionCode);
        });
...
const uploadFile = async (authClient, fileBuffer, fileName, instutionId) => {
  ...
  const fileMetadata = {
      name: fileName,
      parents: [folderStr[instutionId].Student], // Replace with your actual folder ID
  };
